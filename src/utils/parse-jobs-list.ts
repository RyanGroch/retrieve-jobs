...
export const parseJobsList = (rawJobs: string): JobListItem[] =>
  rawJobs
    // Each job is separated by a newline
    .split("\n")

    // Assume at least one space between each column.
    // Assume User ID is in the first column and Job ID is in the second.
    .map((row) => {
      const cols = row.split(" ").filter((col) => col);
      return { userID: cols[0], jobID: cols[1] };
    })
    ...
    .filter(
      ({ jobID }) => jobID && jobID.length === 8 && !jobID.startsWith("TSU")
    );
